...
def test_parse_file_referenceFunction() -> None:
    ...
    includedProcesses = "Building materials required for a compost plant and its construction as well as the disposal" + \
                        "of these materials were included. Land use during construction and use is considered. The " + \
                        "lifetime of the plant was assumed as 25 years. Transport of the building materials to the " + \
                        "construction site were included."
    generalComment = "The inventory refers to a compost plant over the lifetime of 25 years. The compost plant is " + \
                     "constructed for a treating capactiy of 10‘000 tons biogenic waste per year. The total turnover of " + \
                     "the plant over the entire lifetime of 25 years amounts thus 250‘000 tons biogenic waste."
    formula = "0" 
    CASNumber = "007439-89-6"
    referenceFunction = parse_file(
        StringIO(f"""
            <referenceFunction name="{name}" localName="{localName}" infrastructureProcess="true" unit="{unit}"
                category="{category}" subCategory="{subCategory}" localCategory="{localCategory}"
                localSubCategory="{localSubCategory}" amount="1" includedProcesses="{includedProcesses}"
                generalComment="{generalComment}" formula="{formula}" infrastructureIncluded="true"
                datasetRelatesToProduct="true" CASNumber="{CASNumber}" statisticalClassification="100">
                <synonym>0</synonym>
            </referenceFunction>"""
        )
    )
    ...
    assert referenceFunction.infrastructureProcess == True
    ...
    assert referenceFunction.infrastructureIncluded == True
    assert referenceFunction.datasetRelatesToProduct == True
...
def test_parse_file_geography() -> None:
    ...
    geography = parse_file(StringIO(f"<geography location='{location}' text='{text}' />"))
...
def test_parse_file_dataSetInformation() -> None:
    ...
    dataSetInformation = parse_file(
        StringIO(f"""
            <dataSetInformation type="1" impactAssessmentResult="false"
            timestamp="2003-09-12T10:14:36" version="1.3" internalVersion="53.03"
            energyValues="0" languageCode="{languageCode}" localLanguageCode="{localLanguageCode}" />"""
        )
    )

    assert dataSetInformation.type == 1
    assert dataSetInformation.typeStr == "Unit process"
    assert dataSetInformation.impactAssessmentResult == False
...
def test_parse_file_validation() -> None:
    ...
    validation = parse_file(
        StringIO(f"""<validation proofReadingDetails="{proofReadingDetails}" proofReadingValidator="322" 
        otherDetails="{otherDetails}"/>"""
        )
    )
...
def test_parse_file_source() -> None:
    ...
    source = parse_file(
        StringIO(f"""
        <source number="146" sourceType="4" firstAuthor="{firstAuthor}" additionalAuthors="{additionalAuthors}"
        year="2003" title="{title}" titleOfAnthology="{titleOfAnthology}" placeOfPublications="{placeOfPublications}"
        publisher="{publisher}" volumeNo="15" text="{text}" pageNumbers="{pageNumbers}" nameOfEditors="{nameOfEditors}"
        journal="{journal}" issueNo="{issueNo}"/>"""
        )
    )
...
def test_parse_file_dataGeneratorAndPublication() -> None:
    ...
    dataGeneratorAndPublication = parse_file(
        StringIO(f"""
            <dataGeneratorAndPublication person="309" dataPublishedIn="2" referenceToPublishedSource="146"
            copyright="true" accessRestrictedTo="0" companyCode="{companyCode}" countryCode="{countryCode}"
            pageNumbers="{pageNumbers}"/>"""
        )
    )
    ...
    assert dataGeneratorAndPublication.dataPublishedInStr == "Data has been published entirely in 'referenceToPublishedSource'"
    assert dataGeneratorAndPublication.referenceToPublishedSource == 146
    assert dataGeneratorAndPublication.copyright == True
...
def test_parse_file_representativeness() -> None:
    ...
    representativeness = parse_file(
        StringIO(f"""
            <representativeness percent="84.0" productionVolume="{productionVolume}" samplingProcedure="{samplingProcedure}"
            extrapolations="{extrapolations}" uncertaintyAdjustments="{uncertaintyAdjustments}" />"""
        )
    )
...
def test_parse_file_timePeriod() -> None:
    ...
    assert timePeriod.dataValidForEntirePeriod == True
...
def test_parse_file_dataEntryBy() -> None:
    dataEntryBy = parse_file(StringIO(f"""<dataEntryBy person="309" qualityNetwork="1" />"""))
